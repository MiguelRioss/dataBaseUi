...
import { getArchivedOrders } from "../services/orderServices.mjs";

export default function ArchivedOrders({ className = "" }) {
  ...
  React.useEffect(() => {
    (async () => {
      const data = await getArchivedOrders(); // -> array of rows
      setRows(sortRows(data, sort));
      setSelectedIds(new Set());
    })();
  }, []); // initial load

  // Re-sort whenever sort changes
  React.useEffect(() => {
  ...
  // If you need to inject handlers into OrderRow later, do it here.
  const rowProps = () => ({});

  return (
    <div className={className}>
      <h2 className="text-xl font-semibold">Archived Orders</h2>
